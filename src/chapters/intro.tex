...
\section{Objectives}
\begin{itemize}
    \item To reconstruct real-time 3D arm and hand trajectories from three RGB webcams using confidence-weighted triangulation.
    \item To map the reconstructed trajectories onto a simulated robotic arm via Inverse Kinematics and export the motion as a structured CSV dataset.
\end{itemize}
...
\begin{itemize}
    \item The system captures only upper-limb motion (shoulder to fingertip) for a single person, and does not replicate finger articulation in PyBullet due to standard URDF limitations, nor support robot policy training beyond dataset generation.
    \item Accuracy depends on proper camera calibration and may degrade under poor lighting, significant self-occlusion, or dynamic camera repositioning without recalibration.
\end{itemize}
